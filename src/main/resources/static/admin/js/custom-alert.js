        ...
        if (status == '' || status == 'success') {
            ...
            confirmbtn == true ?
             ...
             Ok
               ...
               `
                ;
            alert_heading.style = 'background: linear-gradient(80deg, rgb(107 176 252), rgb(211 64 64));';
...
function show_confirm_alert_disable_account(button) {
    const id = 'form'+button.id;
    New.alert({
        status: 'info',
        title: 'Need your confirmation',
        content: 'Do you really want to disable this account?',
        confirmbtn: true
    });
    //if "I approve" is clicked, submit the form
    document.querySelector('.alert_footer .accept').onclick = function () {
        document.getElementById(id).submit();
    };
}

function show_confirm_alert_enable_account(button) {
    const id = 'form'+button.id;
    New.alert({
        status: 'info',
        title: 'Need your confirmation',
        content: 'Do you really want to enable this account?',
        confirmbtn: true
    });
    // if "I approve" is clicked, submit the form
    document.querySelector('.alert_footer .accept').onclick = function () {
...
function show_confirm_alert_disable_service() {
    New.alert({
        status: 'info',
        title: 'Need your confirmation',
        content: 'Do you really want to disable the service(s)?',
        confirmbtn: true
    });
    document.querySelector('.alert_footer .accept').onclick = function () {
...
function show_confirm_alert_enable_service() {
    New.alert({
        status: 'info',
        title: 'Need your confirmation',
        content: 'Do you really want to enable the service(s)?',
        confirmbtn: true
    });
    document.querySelector('.alert_footer .accept').onclick = function () {
        document.getElementById("myForm").submit();
...
function show_Err_alert() {
    New.alert({
        status: 'error',
        title: 'Reached maximum number of service',
        content: 'You have selected the maximum number of services allowed. You cannot choose any more services.',
    })
}
